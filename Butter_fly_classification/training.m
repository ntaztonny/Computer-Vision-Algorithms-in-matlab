imgSet = imageSet('training/','recursive');
descriptors = [];
[x, siz ]  = size(imgSet);
...
for iterator=1:siz
 ...
 for counter=1:imgSet(iterator).Count
    ...
    %save the descriptors for the machine learning algorithm
    descriptors( (iterator - 1)*30 +  counter,:) = [  mean(t1) std(t1) mean(t2) std(t2) mean(t3) std(t3) mean(z1) std(z1) mean(z2) std(z2) mean(z3) std(z3)     mean(w1) std(w1) mean(w2) std(w2) mean(w3) std(w3)  ]
    especies((iterator - 1)*30 + counter, 1) = [iterator]
  end
end                                  
